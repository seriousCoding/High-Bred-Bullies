...
- June 24, 2025. FIXED: Blog and social post generation issues - resolved database schema mismatches (added is_public column to social_posts, fixed blog category constraints), corrected password reset email URL format, and created dedicated blog-routes.js for proper authentication and content generation. Generate blog post and generate social post functions now working correctly.
- June 24, 2025. COMPLETED: Database schema fixes and route integration - added automatic schema updates on server startup to fix is_public column and blog category constraints, properly integrated all modular routes (blog, admin, inquiry, litter, auth) into main server. All admin dashboard functionality now operational with proper authentication and database compatibility.oinbase imports and references, replaced corrupted files with clean JWT-only authentication versions. High Bred Bullies platform now operates exclusively as a dog breeding management system with no cryptocurrency functionality.
