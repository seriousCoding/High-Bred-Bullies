...
- June 24, 2025. COMPLETED: Database schema alignment and profile API fix - identified mismatch between actual database structure (UUID-based from CSV schema) and code expectations (serial-based), updated shared schema to match real database with UUID primary keys and additional fields (breeding_experience, favorite_breeds, etc.), fixed profile API to handle schema differences by providing JWT-based profile data, resolved database inconsistency issues causing profile lookup failures
