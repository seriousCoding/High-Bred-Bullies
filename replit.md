...
- June 24, 2025. FIXED: Authentication and password reset system - added password_hash column to user_profiles table, created proper admin user (gpass1979@gmail.com) with hashed password authentication, corrected database schema references, implemented token storage and validation. Both login and password reset now use proper database-backed authentication with email notifications.oinbase imports and references, replaced corrupted files with clean JWT-only authentication versions. High Bred Bullies platform now operates exclusively as a dog breeding management system with no cryptocurrency functionality.
