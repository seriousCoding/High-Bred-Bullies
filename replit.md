...
- June 24, 2025. COMPLETED: Inquiry delete functionality implemented - added DELETE /api/inquiries/{id} endpoint to auth-server.cjs with proper authentication and database cleanup. Admin can now delete unwanted inquiries from the management panel.oinbase imports and references, replaced corrupted files with clean JWT-only authentication versions. High Bred Bullies platform now operates exclusively as a dog breeding management system with no cryptocurrency functionality.
