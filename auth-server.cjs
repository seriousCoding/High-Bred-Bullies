    ...
    try {
      ...
      if (pathname === '/api/seed-stripe-test-litters' && req.method === 'POST') {
        ...
        try {
          ...
          const breederQuery = await pool.query('SELECT user_id FROM breeders WHERE user_id = $1', [userId]);
          let breederId = userId;
      ...
      if (pathname === '/high-bred-bullies-project.tar.gz' && req.method === 'GET') {
        const fs = require('fs');
        const path = require('path');
        
        try {
          const filePath = path.join(__dirname, 'high-bred-bullies-project.tar.gz');
          
          if (!fs.existsSync(filePath)) {
            res.writeHead(404);
            res.end('Project archive not found');
            return;
          }
          ...
          res.writeHead(200, {
            ...
            'Content-Disposition': 'attachment; filename="high-bred-bullies-project.tar.gz"',
      ...
      // File info API
      if (pathname === '/api/file-info' && req.method === 'GET') {
        const fs = require('fs');
        const path = require('path');
        ...
        try {
          ...
          res.writeHead(500);
          res.end(JSON.stringify({ error: 'Failed to get file info' }));
        }
        return;
      }
      ...
      if (pathname === '/api/file-structure' && req.method === 'GET') {
        ...
        try {
          const structure = execSync(`find . -maxdepth 3 -type f \\( -name "*.js" -o -name "*.ts" -o -name "*.tsx" -o -name "*.json" -o -name "*.md" \\) | grep -v node_modules | sort | head -20`, 
          ...
          res.writeHead(200);
          res.end(JSON.stringify({
            structure: 'Could not generate file structure'
          }));
        }
        return;
      }
